...
public class PlayerFace
{
	...
	public void tickCamera()
	{
		...
		if (Mouse.isButtonDown(0))
		{
			...
			float angrady = this.camYaw / 180.0F * 3.14F;
			float cosy = (float) Math.cos(angrady);
			float siny = (float) Math.sin(angrady);
			float angradp = this.camPitch / 180.0F * 3.14F;
			float cosp = (float) Math.cos(angradp);
			float sinp = (float) Math.sin(angradp);
			double dX = 0.0D;
			double dZ = 0.0D;
			double dY = 0.0D;
			dX -= dx * cosy;
